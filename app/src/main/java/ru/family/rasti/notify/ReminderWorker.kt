...
class ReminderWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
    ...
    override suspend fun doWork(): Result {
        ...
        checkFeeding(notifier, data, now)
        checkVitamin(notifier, data, now)
        maybeSyncInBackground(notifier, store, data)
        return Result.success()
    }

    private fun checkFeeding(notifier: ReminderNotifier, data: AppData, now: LocalDateTime) {
    ...
    private fun checkVitamin(notifier: ReminderNotifier, data: AppData, now: LocalDateTime) {
    ...
    private fun maybeSyncInBackground(notifier: ReminderNotifier, store: LocalStore, before: AppData) {
        ...
        if (updates.isNotEmpty()) notifier.notifySyncUpdates(updates)
    }
}
